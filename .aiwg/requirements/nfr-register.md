...
| NFR-6.4 | Rebuilding the custom Ubuntu ISO is documented | `docs/build-guide.md` contains a complete Cubic walkthrough sufficient for the maintainer to rebuild from a fresh Ubuntu 24.04 host | Dry-run walkthrough against a clean VM before each major release | HIGH | requirements.md NFR-6.4 |
...
- **Reproducible-build guarantee for the custom Ubuntu ISO.** Cubic is an interactive chroot-editing tool; byte-identical rebuilds are not achievable without replacing the build toolchain. The project accepts this limitation and documents it rather than promising reproducibility. Revisit if we migrate to a declarative builder (live-build, debos, Kiwi) or if supply-chain demands escalate.
