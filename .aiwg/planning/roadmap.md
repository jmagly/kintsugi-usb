...
The 2026-05-24 build/imaging audit (`audit-2026-05-24`, Gitea #34–#42) is the defining event since the last snapshot. It found that the shipped toolkit, while feature-rich, **could not satisfy its own iteration-1 acceptance gate**: the build produces a single `live-build` ISO with no Ventoy assembly and no persistence, and the wizard stops at an `.iso` rather than the `.img.zst` the gate requires. Triage resolved every finding (decisions recorded on each issue); `iteration-001-plan.md` was amended in place to match.
...
| Work | Issue | Notes |
|------|-------|-------|
| **Hardware acceptance round-trip** — fresh clone → wizard → `.img.zst` → flash a real USB → boot a test host → `usb-test-harness.sh` PASS incl. **TC-6 persistence** → `--from-profile` reproduces → capture measured timing/disk figures → backfill `docs/wizard-guide.md` → record in `.aiwg/reports/`. | #37 | **Requires hardware** (Ubuntu 24.04 host + sudo + ~20 GB, a physical USB, physical boot). The "handle when back" linchpin. Unblocks the release. |
