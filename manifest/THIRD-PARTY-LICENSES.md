...
| **Ubuntu 24.04 LTS Server** | 24.04.x | Aggregated (primarily GPL/LGPL/MIT/BSD) | https://ubuntu.com/ | Base OS for the custom ML-Support ISO. Canonical aggregates hundreds of per-package licenses; built with live-build (ADR-007) from Ubuntu packages. |
...
| **Ollama** | per-release (pinned in wizard) | MIT | https://github.com/ollama/ollama | Second local LLM runtime alongside llama.cpp. Installed via Ollama's published install script in the live-build chroot; pinned version, auto-update disabled. |
